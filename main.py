...
def save_data():
    safe_history = [
        ...
        and "user_id" in entry
        and "mode" in entry
        and "players" in entry
    ]
...
class HistoryDeleteButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        save_data()
        await update_leaderboard(interaction.guild)
        await interaction.response.send_message(f"✅ Eintrag **#{self.index + 1}** ({mode}) wurde gelöscht.", ephemeral=True)
...
# (Korrigierte und vollständige main.py – identisch mit vorheriger Version, aber Zeile 197+ korrigiert)
# Nur der relevante Teil ist angepasst – der Rest bleibt wie oben, damit du nicht wieder alles hochladen musst.

# Hier der korrigierte history_cmd-Befehl:
@bot.command(name="history")
async def history_cmd(ctx):
    user_id = ctx.author.id
    user_history = [entry for entry in history if entry["user_id"] == user_id]
    if not user_history:
        await ctx.send("📭 Du hast noch keine Siege eingetragen.", ephemeral=True)
        return
